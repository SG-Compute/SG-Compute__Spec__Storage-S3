# ═══════════════════════════════════════════════════════════════════════════════
# SG/Compute Specs — S3 Server: S3_Server__Service
# Tier-1 pure-logic orchestrator for the S3 server compute spec.
# ═══════════════════════════════════════════════════════════════════════════════

import time

from typing                                                                         import Optional

from osbot_utils.type_safe.Type_Safe                                                import Type_Safe

from sgraph_ai_service_playwright__cli.core.event_bus.Event__Bus                    import event_bus
from sgraph_ai_service_playwright__cli.core.event_bus.schemas.Schema__Stack__Event  import Schema__Stack__Event

from sg_compute_specs.s3_server.collections.List__Schema__S3_Server__Info          import List__Schema__S3_Server__Info
from sg_compute_specs.s3_server.enums.Enum__S3_Server__Stack__State                import Enum__S3_Server__Stack__State
from sg_compute_specs.s3_server.schemas.Schema__S3_Server__Create__Request         import Schema__S3_Server__Create__Request
from sg_compute_specs.s3_server.schemas.Schema__S3_Server__Create__Response        import Schema__S3_Server__Create__Response
from sg_compute_specs.s3_server.schemas.Schema__S3_Server__Delete__Response        import Schema__S3_Server__Delete__Response
from sg_compute_specs.s3_server.schemas.Schema__S3_Server__Health__Response        import Schema__S3_Server__Health__Response
from sg_compute_specs.s3_server.schemas.Schema__S3_Server__Info                    import Schema__S3_Server__Info
from sg_compute_specs.s3_server.schemas.Schema__S3_Server__List                    import Schema__S3_Server__List
from sg_compute_specs.s3_server.service.S3_Server__AWS__Client                     import S3_SERVER_NAMING


DEFAULT_REGION        = 'eu-west-2'
DEFAULT_INSTANCE_TYPE = 't3.small'
PROFILE_NAME          = 's3-server-ec2'
S3_STACK_TYPE_ID      = 's3_server'                                                 # used in event_bus emit


class S3_Server__Service(Type_Safe):
    aws_client        : object = None
    health_checker    : object = None
    mapper            : object = None
    ip_detector       : object = None
    name_gen          : object = None
    user_data_builder : object = None

    def setup(self) -> 'S3_Server__Service':
        from sg_compute_specs.s3_server.service.Caller__IP__Detector           import Caller__IP__Detector
        from sg_compute_specs.s3_server.service.S3_Server__AWS__Client         import S3_Server__AWS__Client
        from sg_compute_specs.s3_server.service.S3_Server__Health__Checker     import S3_Server__Health__Checker
        from sg_compute_specs.s3_server.service.S3_Server__Instance__Helper    import S3_Server__Instance__Helper
        from sg_compute_specs.s3_server.service.S3_Server__Stack__Mapper       import S3_Server__Stack__Mapper
        from sg_compute_specs.s3_server.service.S3_Server__User_Data__Builder  import S3_Server__User_Data__Builder
        from sg_compute_specs.s3_server.service.Random__Stack__Name__Generator import Random__Stack__Name__Generator
        self.aws_client        = S3_Server__AWS__Client()  .setup()
        self.mapper            = S3_Server__Stack__Mapper()
        self.ip_detector       = Caller__IP__Detector()
        self.name_gen          = Random__Stack__Name__Generator()
        self.user_data_builder = S3_Server__User_Data__Builder()
        checker                = S3_Server__Health__Checker()
        checker.instance       = S3_Server__Instance__Helper()
        self.health_checker    = checker
        return self

    def create_stack(self, request: Schema__S3_Server__Create__Request, creator: str = '') -> Schema__S3_Server__Create__Response:
        t0         = time.monotonic()
        stack_name = str(request.stack_name)     or self.name_gen.generate()
        region     = str(request.region)         or DEFAULT_REGION
        caller_ip  = str(request.caller_ip)      or str(self.ip_detector.detect())
        ami_id     = str(request.from_ami)       or self.aws_client.ami.latest_al2023_ami_id(region)
        itype      = str(request.instance_type)  or DEFAULT_INSTANCE_TYPE
        mode       = request.mode
        backend    = request.backend
        aws_region_target = str(request.aws_region_target) or region

        sg_id     = self.aws_client.sg.ensure_security_group(region, stack_name, caller_ip)
        tags      = self.aws_client.tags.build(stack_name, caller_ip, mode, creator)
        user_data = self.user_data_builder.render(stack_name       = stack_name          ,
                                                  region           = region              ,
                                                  mode             = mode.value          ,
                                                  backend          = backend.value       ,
                                                  aws_region_target= aws_region_target   ,
                                                  max_hours        = request.max_hours   )
        iid       = self.aws_client.launch.run_instance(region, ami_id, sg_id, user_data, tags,
                                                        instance_type         = itype        ,
                                                        instance_profile_name = PROFILE_NAME )
        info      = Schema__S3_Server__Info(
            stack_name        = stack_name                                    ,
            aws_name_tag      = S3_SERVER_NAMING.aws_name_for_stack(stack_name),
            instance_id       = iid                                           ,
            region            = region                                        ,
            ami_id            = ami_id                                        ,
            instance_type     = itype                                         ,
            security_group_id = sg_id                                         ,
            allowed_ip        = caller_ip                                     ,
            state             = Enum__S3_Server__Stack__State.PENDING         ,
            mode              = mode                                           ,
            backend           = backend                                        )
        event_bus.emit('s3_server:stack.created', Schema__Stack__Event(
            type_id     = S3_STACK_TYPE_ID,
            stack_name  = stack_name      ,
            region      = region          ,
            instance_id = str(iid)        ))
        return Schema__S3_Server__Create__Response(
            stack_info = info                              ,
            message    = f'Instance {iid} launching'      ,
            elapsed_ms = int((time.monotonic()-t0)*1000)  )

    def list_stacks(self, region: str) -> Schema__S3_Server__List:
        region = region or DEFAULT_REGION
        raw    = self.aws_client.instance.list_stacks(region)
        stacks = List__Schema__S3_Server__Info()
        for details in raw.values():
            stacks.append(self.mapper.to_info(details, region))
        return Schema__S3_Server__List(region=region, stacks=stacks, total=len(stacks))

    def get_stack_info(self, region: str, stack_name: str) -> Optional[Schema__S3_Server__Info]:
        details = self.aws_client.instance.find_by_stack_name(region, stack_name)
        return self.mapper.to_info(details, region) if details else None

    def delete_stack(self, region: str, stack_name: str) -> Schema__S3_Server__Delete__Response:
        t0      = time.monotonic()
        details = self.aws_client.instance.find_by_stack_name(region, stack_name)
        if not details:
            return Schema__S3_Server__Delete__Response(
                stack_name = stack_name                      ,
                message    = 'stack not found'               ,
                elapsed_ms = int((time.monotonic()-t0)*1000) )
        iid = details.get('InstanceId', '')
        ok  = self.aws_client.instance.terminate_instance(region, iid)
        if ok:
            event_bus.emit('s3_server:stack.deleted', Schema__Stack__Event(
                type_id     = S3_STACK_TYPE_ID,
                stack_name  = stack_name      ,
                region      = region          ,
                instance_id = iid             ))
        return Schema__S3_Server__Delete__Response(
            stack_name = stack_name                                              ,
            deleted    = ok                                                      ,
            message    = f'terminated {iid}' if ok else 'terminate failed'      ,
            elapsed_ms = int((time.monotonic()-t0)*1000)                         )

    def health(self, region: str, stack_name: str,
               timeout_sec: int = 300, poll_sec: int = 10) -> Schema__S3_Server__Health__Response:
        return self.health_checker.check(region, stack_name, timeout_sec=timeout_sec, poll_sec=poll_sec)
